...
import 'package:carpool_demo/Widgets/TransactionList.dart';
...
class _BillingScreenState extends State<BillingScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    firebaseDatabase.child("users/${firebaseAuth.currentUser?.uid}/creditCards").onChildChanged.listen((event) {
      if(mounted){
        setState(() {});
      }
    });
      ...
      body: Container(
        ...
        child: Column(
          children: [
                  ...
                  if(snapshot.hasError){
                    return Center(
                      child: ElevatedButton(
                        ...
                        onPressed: ()async{
                          displayBillingBottomSheet(context, 'user');
                        }, 
                        child: Text(
                          "Add Credit Card",
                          style: TextStyle(
            ...
            SizedBox(height: screenHeight * .01),
            TransactionList(transactionLog: transactionLog,)
          ],
        ), 
      )
    );
