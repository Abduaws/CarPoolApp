...
class TransactionListDriver extends StatefulWidget {
  final dynamic transactionLog;
  const TransactionListDriver({super.key, this.transactionLog});

  @override
  State<TransactionListDriver> createState() => _TransactionListDriverState();
}

class _TransactionListDriverState extends State<TransactionListDriver> {
  ...
  Future<List<Map<Object?, Object?>>> getTransactionList() async {
    ...
    DataSnapshot dataSnapshot = await firebaseDatabase.child("drivers/${firebaseAuth.currentUser?.uid}/transactionLog").get();
  ...
  Widget build(BuildContext context) {
    ...
    firebaseDatabase.child("drivers/${firebaseAuth.currentUser?.uid}/transactionLog").onChildChanged.listen((event) {
      if(mounted){
        setState(() {});
      }
    });
